...
# ---------------------------------------------------------------------------
# Aşama 2 – Gerçek mamba-ssm kütüphanesi (CUDA selective-scan)
# ---------------------------------------------------------------------------

class TDL_Mamba(nn.Module):
    """
    Aşama 2 modeli: 2× (pre-norm + Mamba + residual) bloğu.
    ...
    def __init__(self):
        ...
        self.mamba_1           = Mamba(d_model=1024, d_state=16, d_conv=4, expand=2)
        self.dropout           = nn.Dropout(0.3)
        self.norm_2            = nn.LayerNorm(1024)
        self.mamba_2           = Mamba(d_model=1024, d_state=16, d_conv=4, expand=2)
        self.classifier_module = ClassifierModule()
    ...
    def forward(self, x: Tensor):
        ...
        x_t = x_t + self.mamba_1(self.norm_1(x_t))                  # pre-norm + residual
        x_t = self.dropout(x_t)
        x_t = x_t + self.mamba_2(self.norm_2(x_t))                  # pre-norm + residual
        x_t = x_t.transpose(1, 2)                                    # (B, 1024, T)
